...
- **NO comments** (no `/*` or `//`)
- **Path pattern** on first line (e.g., `/*`)
- **Headers** indented with 2 spaces on following lines

**Current format:** ✅ CORRECT (just verified)
...
git commit -m "Fix CSP: Correct _headers file format for Netlify"
git push
```
...
1. After Netlify build completes, check build logs
2. Look for any errors about `_headers`
3. Or check if `dist/_headers` exists in your local build:
   ```bash
   npm run build  # or bun run build
   ls dist/_headers  # Should show the file
   ```
