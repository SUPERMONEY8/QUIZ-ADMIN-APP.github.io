...
export default function QuestionFormShortAnswer({ quizId, onSaved }) {
  const { user } = useAuth();
  ...
  const handleSave = async (e) => {
    ...
    try {
      setSaving(true);
      console.log("Saving Short Answer question...", { quizId, userId: user?.id || user?.uid });
      ...
      const questionData = {
        ...
        correct_answer: expectedAnswer.trim() || "", // Expected answer for reference
      ...
      console.log("✅ Short Answer question saved with ID:", questionId);
      setSuccess(true);
  ...
  return (
    <div className="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6 sm:p-8">
      <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Add Short Answer Question</h2>
      <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">Manual grading required for this question.</p>
      ...
      <form className="mt-6" onSubmit={handleSave}>
        <div className="grid grid-cols-1 gap-5">
          ...
          <div>
            <label htmlFor="expected" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            ...
            <input
              ...
              className="block w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-3 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 placeholder-gray-400 dark:placeholder-gray-300"
              placeholder="Optional hint for graders"
            />
          </div>
