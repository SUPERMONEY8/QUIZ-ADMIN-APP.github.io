// MySQL API Helpers - Replaces Firebase/Supabase
...
const getApiUrl = () => {
  ...
  if (isProduction) {
    ...
    return `${protocol}//${hostname}/api` || `${protocol}//${hostname}/quiz-app-api/api`;
  }
  
  // Development: use localhost
  return 'http://localhost/quiz-app-api/api';
};
...
// API call helper with HTML detection and offline fallback
async function apiCall(endpoint, method = 'GET', data = null) {
  try {
    ...
    if (contentType && !contentType.includes('application/json')) {
      ...
      // If it's HTML, the API endpoint doesn't exist or server error
      if (text.trim().startsWith('<!') || text.trim().startsWith('<html')) {
        console.warn(`API endpoint returned HTML instead of JSON: ${endpoint}`);
        // Return empty result for GET requests, throw for others
        if (method === 'GET') {
          // Check if endpoint typically returns array (quizzes, questions, results)
          const arrayEndpoints = ['quizzes.php', 'questions.php', 'results.php'];
    ...
    if (!response.ok) {
      ...
      try {
        ...
        // If JSON parse fails, it's probably HTML
    ...
    // Network errors or API unavailable - use localStorage fallback for GET requests
    if (error.message.includes('Failed to fetch') || error.message.includes('NetworkError') || error.message.includes('API endpoint not found')) {
      console.warn(`API unavailable, using fallback for ${endpoint}`);
      if (method === 'GET') {
        // Try to get from localStorage
        const storageKey = `quiz_app_cache_${endpoint}`;
...
export const quizOperations = {
  ...
  get: async (quizId) => {
    ...
    try {
      ...
      const data = await apiCall(`quizzes.php/${quizId}?admin_id=${encodeURIComponent(adminId)}`);
  ...
  getByAdmin: async (adminId) => {
    try {
      // If adminId is provided, use it; otherwise get current user's ID
      const userId = adminId || await getAdminId();
      const data = await apiCall(`quizzes.php?admin_id=${encodeURIComponent(userId)}`);
      return data.map(transformQuiz).filter(Boolean);
    } catch (error) {
  ...
  create: async (data) => {
    try {
      ...
      const quizData = {
        ...data,
        admin_id: adminId, // Always set admin_id
      };
      const result = await apiCall('quizzes.php', 'POST', quizData);
      return result.id;
    } catch (error) {
  ...
  update: async (quizId, data) => {
    try {
      ...
      const updateData = {
        ...data,
        admin_id: adminId, // Always include admin_id for security
      };
      await apiCall(`quizzes.php/${quizId}`, 'PUT', updateData);
    } catch (error) {
  ...
  delete: async (quizId) => {
    try {
      const adminId = await getAdminId();
      await apiCall(`quizzes.php/${quizId}?admin_id=${encodeURIComponent(adminId)}`, 'DELETE');
    } catch (error) {
...
export const questionOperations = {
  ...
  getByQuiz: async (quizId) => {
    try {
      const adminId = await getAdminId();
      const data = await apiCall(`questions.php?quiz_id=${encodeURIComponent(quizId)}&admin_id=${encodeURIComponent(adminId)}`);
      return data.map(transformQuestion).filter(Boolean);
    } catch (error) {
  ...
  create: async (data) => {
    try {
      const result = await apiCall('questions.php', 'POST', data);
      return result.id;
    } catch (error) {
  ...
  update: async (questionId, data) => {
    try {
      ...
      await apiCall(`questions.php/${questionId}`, 'PUT', updateData);
    } catch (error) {
  ...
  delete: async (questionId) => {
    try {
      const adminId = await getAdminId();
      await apiCall(`questions.php/${questionId}?admin_id=${encodeURIComponent(adminId)}`, 'DELETE');
    } catch (error) {
...
export const resultOperations = {
  ...
  create: async (data) => {
    try {
      // Results don't need admin_id check on create (public quiz taking)
      const result = await apiCall('results.php', 'POST', data);
      return result.id;
    } catch (error) {
  ...
  getByQuiz: async (quizId) => {
    try {
      const adminId = await getAdminId();
      const data = await apiCall(`results.php?quiz_id=${encodeURIComponent(quizId)}&admin_id=${encodeURIComponent(adminId)}`);
      return data;
    } catch (error) {
